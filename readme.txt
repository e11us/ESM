...
This project started as a note taking system, for better organising and collecting and keeping track of notes ( and thoughts ? ). 
currently version includes the following basic functionalities: different type of dynamic background, a simple file browser, infinity note board, password manager. Calendar, file browser and instant note
...
How to use.
currently the config for the GUI is set for 2K screen, resolution 2560x1440, different screen will work, but some GUI will not display as intended. This will be change in the future update.
ESM used MySQL for storing all the information, so in order to use, a local or online database account will be needed. 
